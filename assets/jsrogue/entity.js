...
Game.Entity.prototype.tryMove = function (x, y, z, map) {
    ...
    // Check if the tile is diggable
        ...
        if (this.hasMixin(Game.EntityMixins.PlayerActor)) {
            if (this.getHp() > 1) {
                this.takeDamage({}, 1);
                if (this.hasMixin(Game.EntityMixins.FoodConsumer)) {
                    this.modifyFullnessBy(-25);
                }
                map.dig(x, y, z);
                Game.sendMessage(this, 'You use up some of your energy to dig at the wall');
                return true;
            }
            else {
                Game.sendMessage(this, 'You have no energy left to dig!');
            }
        }
        // If not nothing we can do, but we can't 
        // move to the tile
